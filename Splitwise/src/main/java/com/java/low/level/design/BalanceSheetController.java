...
public class BalanceSheetController {
    public void updateUserExpenseBalanceSheet(User expensePaidBy, List<Split> splitLists, double totalExpenseAmount) {
        ...
        for(Split split : splitLists) {
            ...
            if(expensePaidBy.getUserId().equals(userOwe.getUserId())) {
                paidByUserBalanceSheet.setTotalMyExpenses(paidByUserBalanceSheet.getTotalMyExpenses()+oweAmount);
            } else {
                //update the balance of paid user
                paidByUserBalanceSheet.setTotalAmountToGetBack(paidByUserBalanceSheet.getTotalAmountToGetBack()+oweAmount);
                ...
                owedByUserBalanceSheet.setTotalOwedAmount(owedByUserBalanceSheet.getTotalOwedAmount()+oweAmount);
                owedByUserBalanceSheet.setTotalMyExpenses(owedByUserBalanceSheet.getTotalMyExpenses() + oweAmount);

                Balance userPaidBalance;
                if(owedByUserBalanceSheet.getUserVsBalance().containsKey(expensePaidBy.getUserId())) {
                    userPaidBalance = owedByUserBalanceSheet.getUserVsBalance().get(expensePaidBy.getUserId());
                } else {
    ...
    public void showBalanceSheetOfUser(User user) {
        ...
        System.out.println("TotalAmountToGetBack : " + userExpenseBalanceSheet.getTotalAmountToGetBack());
